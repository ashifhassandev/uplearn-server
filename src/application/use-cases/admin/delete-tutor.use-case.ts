...
import { IDeleteTutorUseCase } from "../../ports/use-cases/admin/delete-tutor.use-case.interface";

@injectable()
export class DeleteTutorUseCase implements IDeleteTutorUseCase {
  ...
  async execute(input: UpdateUserStatusInputDTO): Promise<void> {
    this.log.attempt(LOG_ACTION.DELETE_TUTOR, { userId: input.userId });
    ...
    if (user.status === UserStatus.DELETED) {
      this.log.failed(
        LOG_ACTION.DELETE_TUTOR,
        LOG_REASONS.ADMIN.ALREADY_DELETED,
        {
          userId: input.userId,
          currentStatus: user.status,
        },
      );
      throw new AppError(ADMIN_ERRORS.ALREADY_DELETED, HttpStatus.CONFLICT);
    }

    await this._userRepository.updateStatus(input.userId, UserStatus.DELETED);

    this.log.success(LOG_ACTION.DELETE_TUTOR, { userId: input.userId });
  }
}