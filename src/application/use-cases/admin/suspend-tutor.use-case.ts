...
@injectable()
export class SuspendTutorUseCase implements ISuspendTutorUseCase {
  ...
  async execute(input: UpdateUserStatusInputDTO): Promise<void> {
    this.log.attempt(LOG_ACTION.SUSPEND_TUTOR, { userId: input.userId });
    ...
    this.log.success(LOG_ACTION.SUSPEND_TUTOR, { userId: input.userId });
  }
}