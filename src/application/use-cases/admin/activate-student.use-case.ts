...
import { IActivateStudentUseCase } from "../../ports/use-cases/admin/activate-student.use-case.interface";

@injectable()
export class ActivateStudentUseCase implements IActivateStudentUseCase {
  ...
  async execute(input: UpdateUserStatusInputDTO): Promise<void> {
    this.log.attempt(LOG_ACTION.ACTIVATE_STUDENT, { userId: input.userId });
    ...
    this.log.success(LOG_ACTION.ACTIVATE_STUDENT, { userId: input.userId });
  }
}